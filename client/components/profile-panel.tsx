...
export const ProfilePanel = ({
  ...
  return (
    <div className="fixed inset-0 z-[320] bg-ink/20 backdrop-blur-sm animate-in fade-in duration-200">
      <div className="surface-elevated absolute right-0 top-0 h-full w-full overflow-y-auto border-l border-ink/8 p-4 pb-[calc(env(safe-area-inset-bottom,0px)+1rem)] sm:max-w-lg sm:p-6">
        ...
        {isLoading || !profile ? (
          ...
          <div className="space-y-5">
            <div className="surface-card glass-sheen rounded-[26px] p-4 sm:rounded-[30px] sm:p-5">
              <p className="soft-heading text-2xl font-semibold uppercase tracking-tight text-ink sm:text-3xl">{profile.name}</p>
            ...
            <div className="surface-card rounded-[26px] p-4 sm:rounded-[30px] sm:p-5">
              <div className="mb-4">
                <p className="text-xs font-semibold uppercase tracking-[0.2em] text-lagoon/70">
                  Communication
                </p>
                <h3 className="soft-heading mt-1 text-xl font-semibold text-ink">Recent call history</h3>
              </div>
              ...
              {callHistory.length === 0 ? (
                ...
                <div className="space-y-3">
                  {callHistory.map((entry) => (
                    <div
                      key={entry._id}
                      className="flex items-start gap-3 rounded-[22px] border border-ink/8 bg-white/75 px-4 py-3 shadow-soft"
                    >
                      <div
                      ...
                      <div className="min-w-0 flex-1">
                        ...
                        <div className="mt-2 flex flex-wrap items-center gap-2 text-xs">
                          ...
                          <span className="rounded-full bg-white/85 px-2.5 py-1 font-medium text-ink/55">
                            {formatDuration(entry.durationSeconds)}
                          </span>
                          <span className="rounded-full bg-white/85 px-2.5 py-1 font-medium text-ink/45">

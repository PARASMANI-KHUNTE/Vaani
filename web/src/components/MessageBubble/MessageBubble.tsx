...
const ReactionPicker = ({
  ...
  >
    <div className="flex items-center gap-1 rounded-2xl border border-ink/10 bg-white/98 p-2 shadow-xl backdrop-blur-sm dark:border-white/8 dark:bg-slate-800/98">
      {REACTION_EMOJIS.map((emoji) => (
        <button
          ...
          className="flex h-9 w-9 items-center justify-center rounded-xl text-lg transition-all hover:scale-125 hover:bg-shell dark:hover:bg-slate-700"
        >
          {emoji}
        </button>
      ))}
      ...
      >
        {Object.entries(groupedReactions).map(([emoji, users]) => (
          <button
            ...
            className={cn(
              "flex items-center gap-1 rounded-full border bg-white/80 px-2 py-0.5 text-sm transition-all hover:scale-105 dark:bg-white/5",
              users.some((u) => u.userId === currentUserId)
                ? "border-lagoon/40 bg-lagoon/10"
                : "border-ink/10 hover:border-lagoon/30 dark:border-white/8"
            )}
          >
            <span>{emoji}</span>
            <span className="text-xs font-medium text-ink/70 dark:text-slate-100/60">{users.length}</span>
          </button>
        ))}
        ...
        <button
          ...
          className={cn(
            "flex h-7 w-7 items-center justify-center rounded-full border border-dashed border-ink/15 text-ink/40 transition-all hover:border-lagoon/30 hover:border-solid hover:text-lagoon dark:border-white/10 dark:text-slate-100/25",
...
export const MessageBubble = ({
  ...
  if (message.isSystem) {
    return (
      ...
      >
        <div className="max-w-[92%] rounded-full border border-ink/10 bg-white/75 px-4 py-1.5 text-center text-xs text-ink/60 shadow-soft dark:border-white/10 dark:bg-white/5 dark:text-slate-300/80">
          {message.content || "Group updated"}
        </div>
      </motion.div>
    );
  }
  ...
  return (
    <motion.div
      ...
      role="article"
      aria-label={`Message from ${isOwnMessage ? "you" : senderName} at ${formatMessageTime(message.createdAt)}`}
    >
      {showSenderName && !isOwnMessage && (
        <span className="mb-1 ml-3 text-[11px] font-medium text-lagoon/70 dark:text-cyan-400/60">
          {senderName}
        </span>
      )}
      <div
        className={cn(
          ...
          isOwnMessage
            ? "rounded-br-md bg-[linear-gradient(135deg,#3730a3_0%,#1d4ed8_45%,#0891b2_100%)] text-white"
            : "rounded-bl-md border border-ink/10 bg-white/88 text-ink backdrop-blur-xl dark:border-cyan-300/10 dark:bg-[linear-gradient(180deg,rgba(12,21,36,0.82),rgba(10,17,30,0.7))] dark:text-slate-100"
        )}
      ...
      >
          ...
          <div
            className={cn(
              ...
              isOwnMessage
                ...
                : "border-ink/10 bg-ink/5 text-ink/75 dark:border-white/10 dark:bg-white/8 dark:text-slate-200/80"
            )}
          >
        ...
        {message.deletedForEveryone ? (
          ...
          <>
            ...
            {message.type === "voice" && message.media?.url ? (
              <div
                className={cn(
                  ...
                  isOwnMessage ? "border-white/10 bg-white/10" : "border-ink/10 bg-ink/5 dark:border-white/12 dark:bg-white/8"
                )}
              >
                  ...
                  <div className="mb-3 flex h-12 items-end gap-1">
                    {message.media.waveform.map((bar, index) => (
                      <span
                        ...
                        className={cn("w-full rounded-full", isOwnMessage ? "bg-white/80" : "bg-cyan-300/75")}
                ...
                <div className="mb-2 flex items-center justify-between text-[11px]">
                  <span className={cn(isOwnMessage ? "text-white/75" : "text-ink/55 dark:text-slate-200/55")}>Voice note</span>
                  <span className={cn(isOwnMessage ? "text-white/75" : "text-ink/55 dark:text-slate-200/55")}>
              ...
              >
                <div
                  className={cn(
                    ...
                    isOwnMessage ? "bg-white/20" : "bg-cyan-500/10 dark:bg-cyan-500/15"
                  )}
                >
                  <Paperclip className={cn("h-6 w-6", isOwnMessage ? "text-white/80" : "text-lagoon dark:text-cyan-300")} />
                </div>
                <div className="min-w-0 flex-1">
                  <p className={cn("truncate text-sm font-medium", isOwnMessage ? "text-white" : "text-ink dark:text-slate-100")}>
                    {message.media.originalName || "File"}
                  </p>
                  {message.media.bytes && (
                    <p className={cn("text-xs", isOwnMessage ? "text-white/60" : "text-ink/55 dark:text-slate-300/50")}>
                ...
                <a
                  ...
                  className={cn(
                    ...
                    isOwnMessage ? "bg-white/20 text-white hover:bg-white/30" : "bg-cyan-500/10 text-lagoon hover:bg-cyan-500/20 dark:bg-cyan-500/15 dark:text-cyan-300 dark:hover:bg-cyan-500/25"
                  )}
                >
                  Open
                </a>
            ...
            {message.content ? (
              <p className="whitespace-pre-wrap break-words leading-6">{renderMessageContent(message.content, isOwnMessage)}</p>
            ) : null}
          </>
        )}
        ...
        <div
          className={cn(
            ...
            isOwnMessage ? "text-white/75" : "text-ink/50 dark:text-slate-200/45"
          )}
        >
          <div className="flex items-center gap-1 opacity-0 transition-all duration-200 group-hover:opacity-100">
            <div className="relative">
              <button
                ...
                className="inline-flex items-center gap-1 rounded-full px-2 py-1 transition hover:bg-ink/10 dark:hover:bg-white/20"
            ...
            <button
              ...
              onClick={() => handleDeleteClick("me")}
                className={cn(
                  ...
                  showDeleteConfirm ? "animate-pulse bg-red-500 text-white" : "hover:bg-ink/10 dark:hover:bg-white/20"
                )}
            ...
            {isOwnMessage && !message.deletedForEveryone ? (
              <button
                ...
                className={cn(
                  "inline-flex items-center gap-1 rounded-full px-2 py-1 transition",
                  showDeleteConfirm ? "animate-pulse bg-red-600 text-white" : "hover:bg-ink/10 dark:hover:bg-white/20"
                )}
          ...
          <div
            className={cn(
              ...
              isOwnMessage ? "justify-end text-white/75" : "justify-start text-ink/50 dark:text-slate-200/45"
            )}
          >
            <span aria-label={`Sent at ${formatMessageTime(message.createdAt)}`}>{formatMessageTime(message.createdAt)}</span>
            {isOwnMessage ? (
              <StatusIcon
